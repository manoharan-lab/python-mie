...
def diff_scat_intensity_complex_medium_film(m, x, theta, kdistance):
    '''
    Calculates the differential scattered intensity as a function of scattering
    angle theta when the medium has a non-zero imaginary component of the 
    refractive index. This differential scattered intensity is computed by 
    substituting the scattered electric and magnetic fields (from Fu and Sun)
    into the radial component of the Poynting vector:
    ...
    m: complex relative refractive index
    x: size parameter using the medium's refractive index
    theta: array of scattering angles
    rho: k * distance, where k = 2*np.pi*n_matrix/wavelen, and distance is the
         distance away from the center of the particle. The far-field solution
         is obtained when distance >> radius.
    ...
    # at each angle theta, the distance changes in the case of a film
    zn_trim = np.empty([len(theta), len(an)], dtype=np.complex_)
    bessel_deriv = np.empty([len(theta), len(an)], dtype=np.complex_)
    for t in np.arange(len(theta)):  
        # calculate spherical Bessel function and derivative
        d = kdistance / np.cos(np.pi-theta[t])
        jn = spherical_jn(nstop_array, d)
        yn = spherical_yn(nstop_array, d)
        zn = jn + 1j*yn
        zn_trim[t,:] = zn[1:]
    
        _, xi = mie_specfuncs.riccati_psi_xi(d, nstop)
        xishift = np.concatenate((np.zeros(1), xi))[0:nstop+1]
        xi = xi[1:]
        xishift = xishift[1:]
        bessel_deriv[t,:] = xishift - n*xi/d
    ...
    cos_theta = np.broadcast_to(cos_theta.reshape((-1,1)), [len(theta), len(En)]) 
    En = np.broadcast_to(En, [len(theta), len(En)])
    an = np.broadcast_to(an, [len(theta), len(an)])
    bn = np.broadcast_to(bn, [len(theta), len(bn)])               

    Es_theta = np.sum(En* (1j * an * taus * bessel_deriv/(kdistance/cos_theta) - bn * pis * zn_trim), axis=1) 
    Es_phi = np.sum(En* (-1j * an * pis * bessel_deriv/(kdistance/cos_theta) + bn * taus * zn_trim), axis=1)
    
    Hs_phi = np.sum(En* (1j * bn * pis * bessel_deriv/(kdistance/cos_theta) - an * taus * zn_trim), axis=1)
    Hs_theta = np.sum(En* (1j * bn * taus * bessel_deriv/(kdistance/cos_theta) - an * pis * zn_trim), axis=1) 
...
def integrate_intensity_complex_medium_film(m, x, thickness, theta, k,
                                            number_density, cscat_tot, cabs_tot,
                                            structure_factor, transmission, 
                                            phi_min=Quantity(0, 'rad'), 
                                            phi_max=Quantity(2*np.pi, 'rad')):
    '''
    Calculates the scattering cross section when the medium is absorbing by 
    integrating the differential scattered intensity over scattering angles 
    theta and azimuthal angles phi using trapezoid rule. This function assumes
    a film geometry, and includes the attenuation of the incident light with
    Beer-Lambert's law. 
    ...
    d = Quantity(np.linspace(0.01, thickness.magnitude, 100), thickness.units)
    cos_theta = np.cos(np.pi - theta)
    cos_theta = np.broadcast_to(cos_theta.reshape((-1,1)), [len(theta), len(d)])
    d_array = np.broadcast_to(d, [len(theta), len(d)]) * d.units
    
    distance = d_array / cos_theta
    
#    I_par = np.empty([len(theta), len(d)])
#    I_perp = np.empty([len(theta), len(d)])
#    for di in np.arange(len(d)):
#        I = diff_scat_intensity_complex_medium_film(m, x, theta, (d[di]*k).to('').magnitude)
#        I_par[:,di] = I[0]
#        I_perp[:,di] = I[1]
    d_det = thickness  
    I_par,I_perp = diff_scat_intensity_complex_medium(m, x, theta, 
                                                     (k*d_det).to('').magnitude)
    I_par = np.broadcast_to(I_par.reshape((-1,1)), [len(theta), len(d)])
    I_perp = np.broadcast_to(I_perp.reshape((-1,1)), [len(theta), len(d)])
    
    beer = (1 - np.exp(-number_density * (cscat_tot + cabs_tot) * distance)) * cscat_tot / (cscat_tot + cabs_tot)    
    
    # multiply by factor that accounts for attenuation in the incident light
    # (see Sudiarta and Chylek (2001), eq 10)
    exponent = np.exp(2*d_array*k.imag)
    factor = 1 / (exponent/(2*d_array*k.imag)+(1-exponent)/(2*d_array*k.imag)**2)
    
    structure_factor_par = structure_factor[0]
    structure_factor_perp = structure_factor[1]
    structure_factor_par = np.broadcast_to(structure_factor_par.reshape((-1,1)), [len(theta), len(d)])
    structure_factor_perp = np.broadcast_to(structure_factor_perp.reshape((-1,1)), [len(theta), len(d)])
    
    transmission_par = transmission[0]    
    transmission_perp = transmission[1] 
    transmission_par = np.broadcast_to(transmission_par.reshape((-1,1)), [len(theta), len(d)])
    transmission_perp = np.broadcast_to(transmission_perp.reshape((-1,1)), [len(theta), len(d)])
    
    dsigma_par = I_par * distance**2 * structure_factor_par * factor * transmission_par #* beer
    dsigma_perp = I_perp * distance**2 * structure_factor_perp *factor * transmission_perp #* beer
    
    # Integrate over the thickness of the sample
    integral_d_par = np.trapz(dsigma_par, x=d, axis=1) * dsigma_par.units
    integral_d_perp = np.trapz(dsigma_perp, x=d, axis=1) * dsigma_perp.units
    
    # Integrate over theta 
    integrand_par = np.trapz(integral_d_par * np.abs(np.sin(theta)), x=theta) * dsigma_par.units
    integrand_perp = np.trapz(integral_d_perp * np.abs(np.sin(theta)), x=theta) * dsigma_perp.units
    ...
    sigma = (sigma_par + sigma_perp)/2 
    
    return(sigma, sigma_par, sigma_perp, dsigma_par/2, 
           dsigma_perp/2)
